...
- **结构化输出 + 5 层兜底**：主生成走 `json_schema` 严格模式；LLM 偏离 schema 时自动降级（围栏 → 裸 ECharts option → content-in-option），确保鲁棒
...
- **多轮对话记忆**（简化版）：基于 `memory/chat_memory.py` 的进程内存储，会话级上下文保持
...
├── llm_client.py           # LLM 调用封装（urllib 兼容层 + LangChain ChatOpenAI 封装 + Provider 策略）
...
├── output_parsers/          # 结构化输出解析（重构后）
│   ├── chart_parser.py       # ECharts option 5 层解析兜底
│   └── schema.py             # Pydantic 数据模型（新增）：ColumnSchema / DataUnderstandingResponse /
│                             #   ChartGenerationResponse / ProviderConfig / PipelineEvent
├── prompts/                 # LangChain PromptTemplate
│   ├── __init__.py           # Prompt 注册中心（新增）
│   ├── data_understanding.py
│   ├── chart_type.py
│   └── chart_generation.py
├── chains/                  # LangChain LCEL Chain
│   ├── __init__.py
│   ├── base.py               # StructuredLLMChain + build_llm 工厂（新增）
│   ├── understanding.py      # 数据理解链（重写为 Pydantic 输出版）
│   ├── chart_generation.py   # 图表生成链（重写为 Pydantic 输出版）
│   └── pipeline.py           # 新版 6 阶段流水线（event-based）
├── tools/                   # LangChain Tool 封装
│   ├── knowledge.py
│   ├── preprocessor.py
│   └── chart_selector.py
├── agents/                  # LangChain Agent 模块（简化版）
│   └── dataviz_agent.py
├── memory/                  # 对话记忆
│   ├── __init__.py
│   └── chat_memory.py       # ChatMemory + LangChainChatMessageHistory（重写）
...
**`structured`** 是最理想的 parse_method——LLM 直接返回 Pydantic 对象，零解析损耗，类型安全。前端非 `primary`/`structured` 时顶部显示「⚠ LLM 偏离 schema」红色徽章。
...
### LangChain 集成架构（v2 重构）

LangChain 模块位于 `prompts/` / `chains/` / `tools/` / `agents/` / `memory/` / `output_parsers/`，是核心业务的**可选扩展层**，遵循"降级可运行"原则：

| 模块 | 路径 | 用途 |
|---|---|---|
| **Prompt 注册中心** | `prompts/__init__.py` | 统一从一个模块导入所有 prompt，支持 try/except 降级导入 |
| **Chain 构建器** | `chains/base.py` | `StructuredLLMChain(cfg, system_prompt, output_model)` — 声明式 Chain 装配；自动尝试 `ChatOpenAI.with_structured_output(PydanticModel)` |
| **Pydantic 数据模型** | `output_parsers/schema.py` | `ColumnSchema` / `DataUnderstandingResponse` / `ChartGenerationResponse` / `ProviderConfig` / `PipelineEvent`；替代手写 JSON schema |
| **数据理解链** | `chains/understanding.py` | 输出绑定 `DataUnderstandingResponse`，Pydantic 直接解析，省去手写 JSON 校验 |
| **图表生成链** | `chains/chart_generation.py` | 输出绑定 `ChartGenerationResponse`，失败时自动降级到 5 层兜底 |
| **新版流水线** | `chains/pipeline.py` | event-based 6 阶段生成器，与 `app.py` 内部 pipeline 并行存在 |
| **Tools** | `tools/knowledge.py` 等 | ECharts 知识库 / 数据预处理 / 图表类型选择 Tool，可组合进 Agent |
| **Agent** | `agents/dataviz_agent.py` | 简化版 DataViz Agent（直接调 LLM，不依赖 Tool 调用链路） |
| **Memory** | `memory/chat_memory.py` | `LangChainChatMessageHistory`（类似 `BaseChatMessageHistory`）+ `ChatMemory`（兼容旧接口）+ `SimpleMessage`（可转 `HumanMessage`/`AIMessage`） |

**架构亮点**：

1. **Pydantic 结构化输出**：用 `with_structured_output(ChartGenerationResponse)` 让 LLM 直接返回类型安全的 Pydantic 对象，替代手写 JSON schema dict
2. **ProviderConfig 策略模式**：provider 的 thinking 字段映射（`reasoning_effort` / `thinking`）集中在 `ProviderConfig.for_provider(name)` 一处，新增 provider 只需改一个方法
3. **StructuredLLMChain 声明式装配**：一行完成 `prompt | llm | PydanticOutputParser` 组合，内部自动处理降级
4. **PipelineEvent 统一事件协议**：stage / delta / done / error 全部是 Pydantic 对象，带类型校验

**向后兼容**：`app.py` 的对外函数签名不变，`/api/parse` / `/api/chart` / `/api/chart/stream` 无需调整。
