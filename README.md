...
├── llm_client.py           # LLM 调用封装（urllib 兼容层 + LangChain ChatOpenAI 封装）
...
├── prompts/                 # LangChain PromptTemplate（新增）
│   ├── data_understanding.py
│   ├── chart_type.py
│   └── chart_generation.py
├── chains/                  # LangChain LCEL Chain（新增）
│   ├── understanding.py
│   ├── chart_generation.py
│   └── pipeline.py
├── tools/                   # LangChain Tool 封装（新增）
│   ├── knowledge.py
│   ├── preprocessor.py
│   └── chart_selector.py
├── agents/                  # LangChain Agent 模块（新增，简化版）
│   └── dataviz_agent.py
├── memory/                  # 对话记忆（新增，简化版）
│   └── chat_memory.py
├── output_parsers/          # 结构化输出解析（新增）
│   └── chart_parser.py
...
2) 扫围栏 → 围栏里是 {option, content}             parse_method = fence_full
3) 围栏里只是裸 ECharts option                     parse_method = fence_option
4) 裸对象 + 顶层有 content 字段 → 抽出 content      parse_method = in_option
5) 围栏 + content-in-option 变种                  parse_method = fence_in_option
6) 还不行 → 502 + raw_reply
```

前端非 `primary` 时顶部显示「⚠ LLM 偏离 schema」红色徽章。
...
LangChain 模块位于 `prompts/` / `chains/` / `tools/` / `agents/` / `memory/` / `output_parsers/`，是核心业务的**可选扩展层**：

| 模块 | 路径 | 用途 |
|---|---|---|
| **Prompt** | `prompts/*.py` | `ChatPromptTemplate` 集中管理，便于版本化 |
| **Chain** | `chains/understanding.py` | 数据理解 LCEL 链：`format_input \| prompt \| llm \| StrOutputParser \| parse_output` |
| **Chain** | `chains/chart_generation.py` | 图表生成 LCEL 链（简化版） |
| **Chain** | `chains/pipeline.py` | 新版 6 阶段流水线（event-based，与 app.py 内部 pipeline 并行存在） |
| **Tools** | `tools/knowledge.py` | ECharts 知识库检索 Tool + 搜索 Tool |
| **Tools** | `tools/preprocessor.py` | 数据预处理 Tool |
| **Tools** | `tools/chart_selector.py` | 图表类型选择 Tool |
| **Agent** | `agents/dataviz_agent.py` | 简化版 DataViz Agent（直接调 LLM，不依赖 Tool 调用链路） |
| **Memory** | `memory/chat_memory.py` | 进程内对话记忆（`ChatMemory` 类 + 全局存储） |
| **Output Parsers** | `output_parsers/chart_parser.py` | ECharts option 5 层解析兜底 + 数据理解 JSON 解析 |
